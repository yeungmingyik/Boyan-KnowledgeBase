...
| `feishuPackage:<spaceId>` | `FeishuPackageImporter` | `{ active?, pending?, release? }`; generations contain `{ baseId, packId, count }`; release contains `{ distributionId, sequence, manifestHash }` | Import reconciliation and signed-space protection; activation and assistant bindings commit together after indexing and active conversations finish. |
| `feishuUpdate:<distributionId>` | `FeishuUpdateService` | `{ sequence, manifestHash, installedSequence?, baseId?, lastUpdatedAt? }` | Verified sequence floor and installed version; survives restart and rejects rollback or conflicting manifests after failed downloads. |
