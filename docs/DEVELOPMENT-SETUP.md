...
Windows 合作方交付版；支持继续分发；模型 API Key 由使用者配置。飞书指定发布范围内的资料均可交付。首期包含业务知识和四个智能体入口。
...
| 验收设备 | Windows x64；Windows arm64 按客户设备增加 | 测试负责人 |
...
- Windows 使用 PowerShell 7；构建与验收使用 Windows 环境。
...
确定许可与仓库 → 准备飞书样本和模型 → 验证本地检索与同步 PoC → 开发知识发布与智能体 → Windows 验收与发行。
